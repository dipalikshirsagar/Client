...
function TaskCalendar({ employeeId }) {
  ...
  const getTooltipContent = (date) => {
    ...
    if (!dateTasks || (dateTasks.tasks.length === 0 && dateTasks.upcomingTasks.length === 0)) {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      return date.getTime() === today.getTime() ? "Today (No Tasks)" : "No tasks";
    }
  ...
  const tileContent = ({ date, view }) => {
    ...
    const dateKey = date.toDateString();
    const dateTasks = taskMap[dateKey];
    const hasTasks = dateTasks && (dateTasks.tasks.length > 0 || dateTasks.upcomingTasks.length > 0);
...
const formatDate = (date) => {
  ...
  return (
    <div className="card shadow-sm mt-2 border-0" style={{ borderRadius: "12px", width: "100%", maxHeight: "auto" }}>
      <h4 className="text-center mt-3" style={{ color: "#3A5FBE", fontSize: "25px", margin: "0px" }}>
        My Task Calendar
      </h4>
      ...
      <Tooltip id="task-calendar-tip" place="top" style={{
      ...
      <div
        ...
        style={{ gap: "15px" }}
      >
        <span><span className="legend-box today-task"></span> Today</span>
        <span><span className="legend-box completed"></span> Completed</span>
        <span><span className="legend-box delayed"></span> Delayed</span>
        <span><span className="legend-box upcoming"></span> Upcoming</span>
      </div>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                {selectedTasks.map((task, index) => {
                  const taskDate = new Date(task.dateOfExpectedCompletion || task.dateOfTaskAssignment);
                  const isToday = taskDate.toDateString() === today.toDateString();
                  const isCompleted = task.status?.name?.toLowerCase() === "completed";
                  const isDelayed = taskDate < today && !isCompleted;
                  const isUpcoming = task.dateOfTaskAssignment && new Date(task.dateOfTaskAssignment) > today && !isCompleted;
                    ...
                    <div key={index} className="container-fluid mb-3">
                      <div className="row p-3 rounded " >
                        <div className="row mb-2">
                          <div className="col-4 fw-semibold" style={{ color: "#212529" }}>Task Name</div>
                        ...
                        {task.projectName?.name && (
                          <div className="row mb-2">
                            <div className="col-4 fw-semibold" style={{ color: "#212529" }}>Project</div>
                            <div className="col-8" style={{ color: "#212529" }}>{task.projectName.name}</div>
                          </div>
                        )}
                        
                        {task.status?.name && (
                          <div className="row mb-2">
                            <div className="col-4 fw-semibold" style={{ color: "#212529" }}>Status</div>
                            <div className="col-8" style={{ color: "#212529" }}>
                        ...
                        {task.progressPercentage !== undefined && (
                          <div className="row mb-2">
                            <div className="col-4 fw-semibold" style={{ color: "#212529" }}>Progress</div>
                            <div className="col-8" style={{ color: "#212529" }}>
                              <span className="progress-percentage">{task.progressPercentage}%</span>
                            </div>
                          </div>
                        )}
                        ...
                        {task.dateOfExpectedCompletion && (
                          <div className="row mb-2">
                            <div className="col-4 fw-semibold" style={{ color: "#212529" }}>Due Date</div>
                            <div className="col-8" style={{ color: "#212529" }}>
                        ...
                        {task.dateOfTaskAssignment && (
                          <div className="row mb-2">
                            <div className="col-4 fw-semibold" style={{ color: "#212529" }}>Assigned Date</div>
                            <div className="col-8" style={{ color: "#212529" }}>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body text-center py-5">
                <div className="no-task-icon mb-3">
                  <i className="bi bi-calendar-check" style={{ fontSize: "3rem", color: "#3A5FBE" }}></i>
                </div>
                <h5 style={{ color: "#3A5FBE" }}>No Tasks</h5>
                <p className="text-muted">You have no tasks assigned for this date.</p>
              </div>
